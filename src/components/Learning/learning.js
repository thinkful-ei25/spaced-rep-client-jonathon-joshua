...
export class Learning extends React.Component {
  ...
  async componentDidMount() {
    await this.props.dispatch(
      fetchProtectedData(this.props.userId, this.props.location.state.category)
    );
  }
  ...
  submitAnswer(e) {
    ...
    let answer = this.props.protectedData.esperantoAnswer
      .replace(/[^\w\s]|_/g, '')
      .replace(/\s+/g, ' ');
    ...
    this.getNextQuestion(
      this.input.value === this.props.protectedData.esperantoAnswer
    );
  }
  ...
  render() {
    ...
    if (this.state.answered !== null) {
      questionField = (
        <h3 className="questionWord">
          {this.state.answered
            ? 'Correct!'
            : `Wrong, the correct answer 
            was ${this.props.protectedData.esperantoAnswer}`}
        </h3>
      );
    } else {
    ...
    this.state.answered !== null
      ? (buttonField = <button className="questionButton">next</button>)
      : (buttonField = <button className="questionButton">Submit</button>);
    const welcomeField = (
      <div className="learningGreet">
        You are Learning: {this.props.location.state.category}
      </div>
    );
    ...
    let logoutButton = (
      <button className="logout" onClick={e => this.logOut(e)}>
        Logout
      </button>
    );
